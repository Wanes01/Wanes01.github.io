<script lang="ts">
	...
	const descriptions: { desc: string; img: string; cls: string }[] = [
		...
		{
			desc: `My thesis involves rebuilding the data layer of Alchemist, an open source physical
		...
		{#each descriptions as description, i}
			<li
				class="flex flex-col items-center justify-between gap-8 lg:flex-row {i % 2 == 1
					...
					: ''}"
			>
				<p use:registerItem class="bg-slate-50/60 lg:w-1/2">{@html description.desc}</p>
